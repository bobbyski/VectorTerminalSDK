...
struct CanvasFrameEventParsingTests {
    ...
    @Test func readEventParsesCanvasResponses() throws {
        ...
        harness.write("\(esc)_VTG;capabilities,protocol=VTG,schema=vtg.capabilities.v1,version=1.5.7,canvasWidth=1440,canvasHeight=1000\(esc)\\")
        #expect(harness.canvas.readEvent(timeoutMilliseconds: 50) == .canvas(VTGCanvas(
            width: 1440,
            height: 1000,
            source: "capabilities",
            rawResponse: "\(esc)_VTG;capabilities,protocol=VTG,schema=vtg.capabilities.v1,version=1.5.7,canvasWidth=1440,canvasHeight=1000\(esc)\\"
        )))
    }
